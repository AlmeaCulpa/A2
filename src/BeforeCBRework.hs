...
data ADT = Expression Expr | Statement [Stat] | Empty
  deriving Show
...
prettyStat (JSConstBlock consts) = prettyConstBlock consts False
...
prettyStatMulti (JSConstBlock consts) = prettyConstBlock consts True
...
prettyIf e (JSCodeBlock s) =
  let str = "if ( " ++ prettyExpr e ++ " ) " ++ prettyCode s False in
  if multiLineCheck str
    then "if ( " ++ prettyExpr e ++ " ) " ++ prettyCode s True
    else str
prettyIf _ _ = 
...
-- >>> parse jsReturn "return parseInt(55);"
-- Result >< JSReturn (JSExprCall "parseInt" [JSInt 55])
